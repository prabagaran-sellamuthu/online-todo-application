	...
	<div class="container col-md-5">
		<div class="card">
			<div class="card-body">
				...
				<c:if test="${todo == null}">
					<form
						action="<%=request.getContextPath()%>/users/${userId}/addtodos"
						method="post">
				</c:if>
				<c:if test="${todo != null}">
					<form
						action="<%=request.getContextPath()%>/users/${userId}/updatetodos/${todo.id}"
						method="post">
				</c:if>
				<caption>
					<h2>
				...
				<fieldset class="form-group">
					<label>Last Updated Date</label> <input type="text"  readonly=true
						value="<c:out value='${todo.lastUpdatedDate}' />" class="form-control disabled"
						name="lastUpdatedDate" required="required">
				</fieldset>
